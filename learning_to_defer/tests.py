...
class PageTests(TestCase):
    def test_overview_loads(self):
        response = self.client.get(
            reverse("learning_to_defer:overview")
        )
        self.assertEqual(response.status_code, 200)

    def test_baseline_page_loads(self):
        response = self.client.get(
            reverse("learning_to_defer:baseline")
        )
...
class PipelineTests(TestCase):
    ...
    def test_pipeline_outputs_probabilities(self):
        ...
        texts = [
            "government election president",
            "football team wins match",
            "stock market company profit",
            "software computer technology",
            "minister country talks",
            "basketball season game",
            "shares bank earnings",
            "internet device science",
        ]
        ...
        self.assertEqual(
            model.predict_proba(["team wins game"]).shape,
            (1, 4),
        )